...
from rollout_policy import MODES, LocalRolloutPolicy
...
def evaluate(scene, mode, world_factory=None):
    ...
    def factory(port, problem, stations, variant, network, mode):
        ...
        policy = ScanEconomyPolicy(*arguments, mode='station_only') if mode == 'previous' else LocalRolloutPolicy(*arguments, mode=mode)
    ...
    row['forecast_events'] = getattr(holder[0], 'forecast_events', [])
    return row, trace
    ...
    for scene in scenes:
        for mode in args.modes:
            ...
            print(scene['profile'], scene['count'], mode,
                  ...
                  'wall', round(row['wall_seconds'], 2), 'changes', row['policy_stats'].get('rollout_probe_changes', 0), flush=True)
    summary = {}
    for mode in args.modes:
