...
ROOT = HERE.parents[1]
sys.path[:0] = [str(HERE), str(ROOT / 'research/selective_optical_v26')]
import run_study
import numpy as np
from anchored_policy import MODES, AnchoredOpticalPolicy, select_preserved_network
...
def hashes():
    return {**run_study.hashes(), **{str(path.relative_to(ROOT)): hashlib.sha256(path.read_bytes()).hexdigest() for path in HERE.glob('*.py')}}
...
def evaluate(scene, mode, stations, world_factory=None):
    ...
    def factory(port, problem, default_stations, variant, network, mode):
        policy = (ShapedProbePolicy(port, problem, default_stations, variant, network, mode='shaped_cost')
                  if mode == 'previous' else AnchoredOpticalPolicy(port, problem, np.asarray(stations), variant, network, mode=mode))
...
def main():
    ...
    network = json.loads(args.network.read_text(encoding='utf-8-sig')) if args.network else select_preserved_network()
    experiment.save(output / 'network.json', network)
    stations = network['stations']
    print('inner_radius', network['inner_radius'], 'outer_coordinates_and_original_order_unchanged', flush=True)
